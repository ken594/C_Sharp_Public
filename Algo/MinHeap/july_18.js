...
class MinHeap {
    ...
    extract() {
        ...
        let i = 1;
        let heapSize = this.heap.length - 1;

        while (i < heapSize) {
            let left = this.idxOfLeftChild(i);
            let right = this.idxOfRightChild(i);

            // if this is the leaf then return
            if (left > heapSize && right > heapSize) {
                return temp;
            }
            // if this is only a left node OR left node is smaller than right node
            // swap the parent node and the left node and update the index
            else if (right > heapSize || this.heap[left] < this.heap[right]) {
                [this.heap[i], this.heap[left]] = [this.heap[left], this.heap[i]];
                i = left;
            }
            // otherwise swap the parent node and the right node
            else {
