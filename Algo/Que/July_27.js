...
const { Stack } = require("../STK/July_24");
...
class Queue {
   ...
   */
    ...
    isSumOfHalvesEqual() {
        ...
        if (queueSize < 2) return true;

        const halfCount = Math.floor(queueSize / 2);
        let leftSum = 0;
        let rightSum = 0;

        for (let i = 0; i < queueSize; i++) {
            if (i < halfCount) {
                // could make a helper function to make code cleaner
                let temp = this.dequeue();
                leftSum += temp;
                this.enqueue(temp);
            }

            if (queueSize % 2 != 0 && i == halfCount) {
                let temp = this.dequeue();
                this.enqueue(temp);
                continue;
            }
...
class TwoStackQueue {
   ...
   */
  dequeue() {
    const stackSize = this.stack1.size();
    let i = 0;
