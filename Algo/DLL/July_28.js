   ...
   */
    ...
    */
    nthToLast(nthLast) {
        ...
        if (nthLast == 1) return this.tail;
        let curr = this.tail;
        while (curr != null && nthLast > 1) {
            curr = curr.prev;
            nthLast--;
        }

        return nthLast > 1 ? null : curr;
    }
    ...
    isPalindrome() {
        // 0 or 1 node
        if (this.isEmpty() || this.head == this.tail) return true;

        let left = this.head;
        let right = this.tail;
        while (left != right || (left && left.prev.data != right.data))
        {
    ...
    isNodeInLeftHalf(node) {
        // 0 or 1 node
        if (this.isEmpty() || this.head == this.tail) return false;
        // 2 nodes
        if (this.head.next == this.tail) return this.head == node;
        // More than 2 nodes
        let slow = this.head;
        let fast = this.head;
        while (fast && fast.next && fast.next.next) {
            if (slow == node) return true;
            slow = slow.next;
            fast = fast.next.next;
        }
...
// console.log(unorderedList.nthToLast(9));

// console.log(firstThreeList.toArray());
// console.log(firstThreeList.isPalindrome());
let testNode = new DLLNode(4);
console.log(unorderedList.isNodeInLeftHalf(testNode));
